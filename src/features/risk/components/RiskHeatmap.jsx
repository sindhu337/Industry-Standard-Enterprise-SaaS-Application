...
/** Returns background color for a given probability×impact cell */
function cellColor(prob, impact) {
  const score = prob * impact
  if (score >= 15) return '#d32f2f'   // critical – dark red
  if (score >= 10) return '#f57c00'   // high – orange
  if (score >= 6)  return '#fbc02d'   // medium – yellow
  if (score >= 3)  return '#689f38'   // low-medium – light green
...
export default function RiskHeatmap({ risks = [] }) {
  // Build lookup: [impact][probability] -> list of risk titles
  const riskMap = {}
  risks.forEach((r) => {
    ...
    <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Box sx={{ display: 'inline-block', minWidth: 320 }}>
          ...
          {/* Rows: impact (Y) from high to low */}
